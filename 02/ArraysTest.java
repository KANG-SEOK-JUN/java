...
public class ArraysTest {
	...
	public static void main(String[] args) {
		...
		//copyOf(배열명, 갯수) : index 0부터 원하는 수 만큼 복사
		int copy1[] = Arrays.copyOf(data,4);
		String copy1Str = Arrays.toString(copy1);	//copy1 배열의 값을 [43,54,32,4]
		System.out.println("copy1->"+copy1Str);
		
		//copyOfRange(배열명 , 시작index, 끝 index+1) : 원하는 위치의 배열을 복사
		int copy2[] = Arrays.copyOfRange(data, 2, 6);
		System.out.println("copy2->"+Arrays.toString(copy2));
		System.out.println("copy2->"+copy2);
		
		//sort() : 배열의 값을 오름차순으로 전체 정렬을 하고 
		//		   원래 배열의 값 순서를 변경하므로 이전 상태로 되돌릴 수 없다.
